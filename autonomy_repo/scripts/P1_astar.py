...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        self.occupancy = occupancy                 # occupancy grid (a DetOccupancyGrid2D object) <- Different occupancy type
    ...
    def is_free(self, x):
        ...
        ### Split the original occupany.is_free into two cases for different occupancy lib
        if isinstance(self.occupancy, DetOccupancyGrid2D):
            if not self.occupancy.is_free(x):
                return False
        elif isinstance(self.occupancy, StochOccupancyGrid2D):
            # Convert tuple x into np.ndarray for calling this lib
            x = np.array(x)
            if not self.occupancy.is_free(x):
                return False
        ### End of the occupancy.is_free check
